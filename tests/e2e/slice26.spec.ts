  ...
  test("keeps a proposal outside canonical extraction until edit-and-accept", async ({ page }) => {
    ...
    try {
      await sql.begin(async (tx) => {
        ...
        await tx`insert into full_text_documents (id, project_id, paper_id, storage_key, original_filename, media_type, byte_size, sha256) values (${documentId}::uuid, ${projectId}::uuid, ${paperId}::uuid, ${`projects/${projectId}/papers/${paperId}/documents/${documentId}/source.pdf`}, 'study.pdf', 'application/pdf', ${bytes.byteLength}, ${documentHash})`;
