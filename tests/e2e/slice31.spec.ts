    ...
    try {
      await sql.begin(async (tx) => {
        ...
        for (const { paper, id, extractionId, pageId } of documents) {
          ...
          await tx`insert into full_text_documents (id, project_id, paper_id, storage_key, original_filename, media_type, byte_size, sha256, storage_state, staged_storage_key) values (${id}::uuid, ${projectId}::uuid, ${paper.id}::uuid, ${`projects/${projectId}/papers/${paper.id}/documents/${id}/source.pdf`}, ${`${paper.title}.pdf`}, 'application/pdf', ${bytes.byteLength}, ${sha256}, 'ready', null)`;
