...
describe("Slice 9 additive upgrade through Slice 10", () => {
  ...
  beforeAll(async () => {
    try {
      ...
      await runStatements(client, fs.readFileSync(path.join(drizzleDir, "0011_deduplication_flow.sql"), "utf8"));
      ready = true;
    } catch { ready = false; }
  }, 60_000);
  afterAll(async () => { if (client) await client.end(); if (ready || client) { const admin = postgres(BASE_URL, { max: 1 }); try { await admin.unsafe(`DROP DATABASE "${TEST_DB_NAME}" WITH (FORCE)`); } finally { await admin.end(); } } }, 60_000);
  it("backfills the legacy question exactly, seeds existing projects, and removes the retired column", async () => {
    ...
    expect(await client`select table_name from information_schema.tables where table_schema='public' and table_name='retrieved_record_deduplication_decisions'`).toEqual([{ table_name: "retrieved_record_deduplication_decisions" }]);
  });
});
