...
describe("Slice 14 migration boundaries", () => {
  ...
  it("creates the document extraction tables and applies all seventeen migrations cleanly", async () => {
    expect(await cleanClient!`select count(*)::integer as count from drizzle.__drizzle_migrations`).toEqual([{ count: 17 }]);
