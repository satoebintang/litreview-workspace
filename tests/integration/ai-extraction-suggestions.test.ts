...
describe("Slice 26 AI extraction suggestion persistence boundary", () => {
  ...
  async function fixture(fieldType: "short_text" | "number" | "single_select" = "number") {
    ...
    await client`insert into full_text_documents (id, project_id, paper_id, storage_key, original_filename, media_type, byte_size, sha256, storage_state, staged_storage_key) values (${documentId}::uuid, ${project.id}::uuid, ${paper.id}::uuid, ${`projects/${project.id}/papers/${paper.id}/documents/${documentId}/source.pdf`}, 'study.pdf', 'application/pdf', ${bytes.byteLength}, ${sha256}, 'ready', null)`;
