...
describe("Slice 4 evidence synthesis", () => {
  ...
  it("rejects duplicate, cross-project, and non-finalized support", async () => {
    ...
    await services.recordFullTextRetrievalAttempt(other.id, otherPaper.id, { outcome: "retrieved", attemptedAt: new Date() });
    await services.recordFullTextScreeningDecision(other.id, otherPaper.id, { decision: "include" });
