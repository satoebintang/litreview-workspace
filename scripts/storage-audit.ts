...
import { auditStorageReadOnly, parseStorageArgs, storageOperationExitCode } from "@/application/storage-operations";
...
async function main() {
  ...
  try {
    const report = await auditStorageReadOnly(db, { documentStorage, intakeStorage, projectId });
...
main().catch((error: unknown) => {
  ...
  const message = error instanceof DomainError
    ? error.message
    : "Storage audit failed; check database and storage configuration.";
