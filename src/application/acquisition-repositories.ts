  ...
  async duplicateCandidates(projectId: string, record: { id: string; doi: string | null; title: string; publicationYear: number | null; sourceRecordId?: string | null }, tx = this.db) {
    return tx.execute(sql`
      ...
      where p.project_id=${projectId} and (
        (${record.doi}::text is not null and btrim(${record.doi}::text) <> '' and p.doi is not null and btrim(p.doi) <> '' and lower(regexp_replace(regexp_replace(btrim(p.doi), '^https?://(dx\\.)?doi\\.org/', '', 'i'), '^doi:[[:space:]]*', '', 'i')) = lower(regexp_replace(regexp_replace(btrim(${record.doi}::text), '^https?://(dx\\.)?doi\\.org/', '', 'i'), '^doi:[[:space:]]*', '', 'i')))
