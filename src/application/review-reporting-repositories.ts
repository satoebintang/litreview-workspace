...
export class ReviewReportingRepository {
  ...
  private metricSql(projectId: string, metric: string) {
    ...
    switch (metric) {
      ...
      case "fullTextRetrievalEligible": return sql`${latestScreening} select p.id, p.title from papers p join current_screening s on s.project_id=p.project_id and s.paper_id=p.id where p.project_id=${projectId} and s.decision='include' order by p.id`;
      case "fullTextNotSought": return sql`${latestScreening} select p.id, p.title from papers p join current_screening s on s.project_id=p.project_id and s.paper_id=p.id where p.project_id=${projectId} and s.decision='include' and not exists (select 1 from full_text_retrieval_attempts r where r.project_id=p.project_id and r.paper_id=p.id) order by p.id`;
      case "fullTextRetrievalPending": case "fullTextRetrieved": case "fullTextUnavailable": return sql`${latestScreening}, latest_retrieval as (select distinct on (project_id,paper_id) project_id,paper_id,outcome from full_text_retrieval_attempts where project_id=${projectId} order by project_id,paper_id,sequence desc) select p.id,p.title,r.outcome from papers p join current_screening s on s.project_id=p.project_id and s.paper_id=p.id join latest_retrieval r on r.project_id=p.project_id and r.paper_id=p.id where p.project_id=${projectId} and s.decision='include' and r.outcome=${metric === "fullTextRetrievalPending" ? "pending" : metric === "fullTextRetrieved" ? "retrieved" : "unavailable"} order by p.id`;
      case "fullTextSought": return sql`${latestScreening} select p.id, p.title from papers p join current_screening s on s.project_id=p.project_id and s.paper_id=p.id where p.project_id=${projectId} and s.decision='include' and exists (select 1 from full_text_retrieval_attempts r where r.project_id=p.project_id and r.paper_id=p.id) order by p.id`;
      case "fullTextEverSought": return sql`select distinct p.id, p.title from papers p join full_text_retrieval_attempts r on r.project_id=p.project_id and r.paper_id=p.id where p.project_id=${projectId} order by p.id`;
      case "fullTextEverRetrieved": return sql`select distinct p.id, p.title from papers p join full_text_retrieval_attempts r on r.project_id=p.project_id and r.paper_id=p.id where p.project_id=${projectId} and r.outcome='retrieved' order by p.id`;
      case "legacyFullTextWithoutRetrieval": return sql`select distinct p.id, p.title from papers p join full_text_screening_decisions f on f.project_id=p.project_id and f.paper_id=p.id where p.project_id=${projectId} and not exists (select 1 from full_text_retrieval_attempts r where r.project_id=p.project_id and r.paper_id=p.id) order by p.id`;
      case "fullTextRetrievalConflicts": return sql`select distinct p.id, p.title from papers p join full_text_retrieval_attempts r on r.project_id=p.project_id and r.paper_id=p.id left join (select distinct on (project_id,paper_id) project_id,paper_id,decision from screening_decisions where project_id=${projectId} and stage='title_abstract' order by project_id,paper_id,sequence desc) s on s.project_id=p.project_id and s.paper_id=p.id where p.project_id=${projectId} and coalesce(s.decision,'') <> 'include' order by p.id`;
  ...
  private mapMetricContributor(metric: string, row: Record<string, unknown>): any {
    ...
    if (metric.startsWith("fullTextRetrieval") || metric === "fullTextNotSought" || metric === "fullTextSought" || metric === "fullTextEverSought" || metric === "fullTextEverRetrieved" || metric === "legacyFullTextWithoutRetrieval") return { kind: "paper", id: stringValue(row.id), label: stringValue(row.title), contribution: 1 };
