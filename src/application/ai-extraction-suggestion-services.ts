...
import { codePointLength } from "@/domain/unicode-offsets";
...
export type AiExtractionSuggestionServices = { beginAiExtractionSuggestion(input: BeginAiExtractionSuggestionInput): Promise<Record<string, unknown>>; executeAiExtractionSuggestion(requestId: string, projectId?: string): Promise<Record<string, unknown>>; expireAiExtractionSuggestion(requestId: string, projectId?: string): Promise<Record<string, unknown>>; getAiExtractionSuggestion(requestId: string, projectId?: string): Promise<Record<string, unknown>>; listAiExtractionSuggestions(projectId: string, paperId: string, fieldId?: string): Promise<Record<string, unknown>[]>; acceptAiExtractionSuggestion(input: AcceptAiExtractionSuggestionInput): Promise<Record<string, unknown>>; rejectAiExtractionSuggestion(projectId: string, requestId: string): Promise<Record<string, unknown>> };
...
export function createAiExtractionSuggestionServices(
  ...
  async function request(requestId: string, executor: Executor = db) { const found = rows(await executor.execute(sql`select * from ai_extraction_requests where id=${requestId}::uuid limit 1`))[0]; if (!found) throw new DomainError("NOT_FOUND", "AI extraction request was not found"); return found; }
  ...
  async function sourcePages(req: Record<string, unknown>, executor: Executor = db) { return rows(await executor.execute(sql`select rp.page_id,rp.page_number,p.text,p.status from ai_extraction_request_pages rp join document_text_extraction_pages p on p.project_id=rp.project_id and p.id=rp.page_id where rp.project_id=${String(req.project_id)}::uuid and rp.request_id=${String(req.id)}::uuid order by rp.page_ordinal`)); }
  ...
  async function persist(requestId: string, providerResult: ProviderSuggestionResult, override?: { outcome: Outcome; diagnostic: string; errorCode: string | null }) {
  ...
  return {
    async beginAiExtractionSuggestion(input) {
      const projectId = id(input.projectId, "Project"), paperId = id(input.paperId, "Paper"), fieldId = id(input.fieldId, "Field"), documentId = id(input.fullTextDocumentId, "Document"), extractionId = id(input.documentTextExtractionId, "Extraction"), key = id(input.idempotencyKey, "Idempotency key");
      ...
      return db.transaction(async (tx) => {
        if (!rows(await tx.execute(sql`select id from papers where project_id=${projectId}::uuid and id=${paperId}::uuid for update`))[0]) throw new DomainError("CROSS_PROJECT_REFERENCE", "Paper does not belong to this project");
        await requireFinallyIncludedPaperLocked(tx, projectId, paperId);
        const field = rows(await tx.execute(sql`select id,name,description,field_type,archived_at from extraction_fields where project_id=${projectId}::uuid and id=${fieldId}::uuid limit 1`))[0]; if (!field) throw new DomainError("CROSS_PROJECT_REFERENCE", "Extraction field was not found"); if (field.archived_at) throw new DomainError("VALIDATION_ERROR", "Archived extraction fields cannot receive AI requests");
        const document = rows(await tx.execute(sql`select id,archived_at from full_text_documents where project_id=${projectId}::uuid and paper_id=${paperId}::uuid and id=${documentId}::uuid limit 1`))[0]; if (!document) throw new DomainError("CROSS_PROJECT_REFERENCE", "Document does not belong to this Paper"); if (document.archived_at) throw new DomainError("DOCUMENT_ARCHIVED", "Archived documents cannot receive AI requests");
        const extraction = rows(await tx.execute(sql`select id,status from document_text_extractions where project_id=${projectId}::uuid and paper_id=${paperId}::uuid and full_text_document_id=${documentId}::uuid and id=${extractionId}::uuid limit 1`))[0]; if (!extraction || !["succeeded", "partial"].includes(String(extraction.status))) throw new DomainError("VALIDATION_ERROR", "Text extraction is not eligible");
        const allPages = rows(await tx.execute(sql`select id,page_number,status,text,character_count from document_text_extraction_pages where project_id=${projectId}::uuid and paper_id=${paperId}::uuid and document_text_extraction_id=${extractionId}::uuid order by page_number`)); const selected = input.pageNumbers?.length ? allPages.filter((page) => input.pageNumbers!.includes(Number(page.page_number))) : allPages.filter((page) => page.status === "succeeded" && Number(page.character_count) > 0); const points = selected.reduce((sum, page) => sum + codePointLength(String(page.text)), 0); const bytes = Buffer.byteLength(selected.map((page) => String(page.text)).join("\n"), "utf8"); if (!selected.length || selected.length > AI_EXTRACTION_LIMITS.maxPages || selected.some((page) => page.status !== "succeeded" || Number(page.character_count) <= 0) || points > AI_EXTRACTION_LIMITS.maxSourceCodePoints || bytes > AI_EXTRACTION_LIMITS.maxSerializedInputBytes) throw new DomainError("VALIDATION_ERROR", "Selected pages are empty, failed, or exceed the AI input limit");
        const options = rows(await tx.execute(sql`select id,label,sort_order from extraction_options where project_id=${projectId}::uuid and field_id=${fieldId}::uuid and archived_at is null order by sort_order,id`)); const baseline = rows(await tx.execute(sql`select r.id from extraction_values v join extraction_value_revisions r on r.project_id=v.project_id and r.extraction_value_id=v.id where v.project_id=${projectId}::uuid and v.paper_id=${paperId}::uuid and v.field_id=${fieldId}::uuid and r.finalized_at is not null order by r.sequence desc limit 1`))[0]?.id ?? null; const requestId = randomUUID(); const model = input.model?.trim() || defaultModel; const reasoning = input.reasoningEffort ?? defaultReasoningEffort; const disclosureVersion = input.disclosureVersion.trim(); if (!model || model.length > 200) throw new DomainError("VALIDATION_ERROR", "AI model configuration is invalid"); if (!(new Set(["none", "minimal", "low", "medium", "high", "xhigh"])).has(reasoning)) throw new DomainError("VALIDATION_ERROR", "AI reasoning configuration is invalid"); if (!disclosureVersion || disclosureVersion.length > 100) throw new DomainError("VALIDATION_ERROR", "AI transmission disclosure version is invalid"); const manifest = selected.map((page) => ({ id: String(page.id), pageNumber: Number(page.page_number), hash: textHash(String(page.text)) }));
        const providerInput: ExtractionSuggestionInput = { field: { id: String(field.id), name: String(field.name), description: field.description == null ? null : String(field.description), fieldType: String(field.field_type) as ExtractionSuggestionInput["field"]["fieldType"], options: options.map((option) => ({ id: String(option.id), label: String(option.label), active: true })) }, pages: selected.map((page) => ({ id: String(page.id), pageNumber: Number(page.page_number), text: String(page.text) })), model, reasoningEffort: reasoning, promptVersion: "extraction-suggestion-v1", responseSchemaVersion: "extraction-suggestion-response-v1", groundingResolverVersion: "exact-page-quote-v1", contextSelectionVersion: "selected-pages-v1", sourceCoverage: { extractionId, documentId, status: String(extraction.status) === "partial" ? "partial" : "succeeded", omittedPageCount: allPages.filter((page) => page.status !== "succeeded" || Number(page.character_count) <= 0).length } };
        ...
        const optionSnapshot = options.map((option) => ({ id: String(option.id), label: String(option.label), sortOrder: Number(option.sort_order) }));
        const intentHash = hash({ projectId, paperId, fieldId, documentId, extractionId, fieldName: String(field.name), fieldDescription: field.description == null ? null : String(field.description), fieldType: String(field.field_type), optionSnapshot, manifest, model, reasoning, promptVersion: "extraction-suggestion-v1", responseSchemaVersion: "extraction-suggestion-response-v1", groundingResolverVersion: "exact-page-quote-v1", contextSelectionVersion: "selected-pages-v1", disclosureVersion });
    ...
    async executeAiExtractionSuggestion(requestId, projectId?) {
      ...
      const claim = await db.transaction(async (tx) => {
        const req = await request(rid, tx);
        if (requestedProjectId && String(req.project_id) !== requestedProjectId) throw new DomainError("CROSS_PROJECT_REFERENCE", "AI request does not belong to this project");
        const found = await result(rid, tx);
        if (found) return { done: true as const, value: mapResult(found), req };
        const dispatch = rows(await tx.execute(sql`select id from ai_extraction_dispatches where project_id=${String(req.project_id)}::uuid and request_id=${rid}::uuid limit 1`))[0];
        if (dispatch) return { done: false as const, pending: true as const, req };
        ...
        return { done: false as const, pending: !inserted, req };
      });
      if (claim.done) return claim.value;
      const replay = pendingPersistence.get(rid);
      if (replay) {
        try {
          const persisted = await persist(rid, replay);
      ...
      if (claim.pending) return this.getAiExtractionSuggestion(rid);
      if (!provider) return persist(rid, { kind: "failure", failure: "api_error", code: "configuration_missing", metadata: { provider: "openai", configuredModel: String(claim.req.configured_model), returnedModel: null, responseId: null, inputTokens: null, outputTokens: null, totalTokens: null, durationMs: 0 } });
      const pages = await sourcePages(claim.req);
      const options = (Array.isArray(claim.req.option_snapshot) ? claim.req.option_snapshot : []) as Array<{ id: string; label: string; sortOrder: number }>;
      const providerInput: ExtractionSuggestionInput = { field: { id: String(claim.req.extraction_field_id), name: String(claim.req.field_name_snapshot), description: claim.req.field_description_snapshot == null ? null : String(claim.req.field_description_snapshot), fieldType: String(claim.req.field_type) as ExtractionSuggestionInput["field"]["fieldType"], options: options.map((option) => ({ id: option.id, label: option.label, active: true })) }, pages: pages.map((page) => ({ id: String(page.page_id), pageNumber: Number(page.page_number), text: String(page.text) })), model: String(claim.req.configured_model), reasoningEffort: String(claim.req.configured_reasoning_effort) as ExtractionSuggestionInput["reasoningEffort"], promptVersion: String(claim.req.prompt_version), responseSchemaVersion: String(claim.req.response_schema_version), groundingResolverVersion: "exact-page-quote-v1", contextSelectionVersion: "selected-pages-v1", sourceCoverage: await sourceCoverage(claim.req) };
      const checkedInput = validateSuggestionInput(providerInput);
      if (!checkedInput.ok) return persist(rid, { kind: "failure", failure: "schema_invalid", code: checkedInput.detail, metadata: { provider: "openai", configuredModel: String(claim.req.configured_model), returnedModel: null, responseId: null, inputTokens: null, outputTokens: null, totalTokens: null, durationMs: 0 } });
      ...
      try {
        ...
        clearTimeout(timer);
        // No normalized result exists, so this application dispatch is
        // terminally uncertain and must never invoke the provider again.
        return persist(rid, { kind: "failure", failure: "transport_error", code: "outcome_unknown", metadata: { provider: "openai", configuredModel: String(claim.req.configured_model), returnedModel: null, responseId: null, inputTokens: null, outputTokens: null, totalTokens: null, durationMs: PROVIDER_TIMEOUT_MS } });
      }
      clearTimeout(timer);
      try {
    ...
    async getAiExtractionSuggestion(requestId, projectId) { const rid = id(requestId, "Request"); const req = await request(rid); if (projectId && String(req.project_id) !== id(projectId, "Project")) throw new DomainError("CROSS_PROJECT_REFERENCE", "AI request does not belong to this project"); const found = await result(rid); const groundings = found ? rows(await db.execute(sql`select * from ai_extraction_result_groundings where project_id=${String(req.project_id)}::uuid and result_id=${String(found.id)}::uuid order by created_at`)) : []; return { request: mapRequest(req), sourceCoverage: await sourceCoverage(req), result: found ? mapResult(found) : null, dispatch: rows(await db.execute(sql`select * from ai_extraction_dispatches where project_id=${String(req.project_id)}::uuid and request_id=${rid}::uuid limit 1`))[0] ?? null, pages: await sourcePages(req), groundings }; },
