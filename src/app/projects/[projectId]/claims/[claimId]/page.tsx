...
import { claimSupportReadServices, reviewServices } from "@/app/server";
import { idSchema } from "@/domain/validation";
import { ConfirmAction } from "@/components";
import { normalizeClaim, normalizeHistory, type ClaimReadServices, type ClaimRevisionView, type ClaimSupportView } from "../model";
...
export default async function ClaimDetailPage({ params, searchParams }: { params: Promise<{ projectId: string; claimId: string }>; searchParams?: Promise<{ error?: string; saved?: string; synthesisRevisionId?: string }> }) {
  ...
  let claim;
  try { claim = await currentClaim(projectId, claimId); } catch (error) { if (error instanceof DomainError && ["PROJECT_NOT_FOUND", "CROSS_PROJECT_REFERENCE", "VALIDATION_ERROR", "NOT_FOUND"].includes(error.code)) notFound(); throw error; }
  const claimHistory = await history(projectId, claimId, claim.currentRevision);
  const current = claim.currentRevision;
  const canEdit = current.state === "active";
  const historicalSource = canEdit ? current : claimHistory.slice().sort((left, right) => right.sequence - left.sequence).find((revision) => revision.state === "active");
