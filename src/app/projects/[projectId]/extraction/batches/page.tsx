...
export default async function ExtractionBatchListPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  return <main className="shell"><header className="topbar"><Link className="brand" href="/"><span className="brand-mark">T</span> Tracework</Link><span className="top-note">Evidence-first literature reviews</span></header>
    <div className="container workspace"><Link className="back-link" href={`/projects/${projectId}/extraction`}>← Extraction protocol</Link>
      <div className="workspace-header"><div><p className="eyebrow">Batch orchestration</p><h1>AI extraction batches</h1><p>Review immutable batch manifests and advance at most two provider requests per action.</p></div><Link className="button" href={`/projects/${projectId}/extraction/batches/new`}>New batch</Link></div>
