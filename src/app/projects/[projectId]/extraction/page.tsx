  ...
  return <main className="shell"><header className="topbar"><Link className="brand" href="/"><span className="brand-mark">T</span> Tracework</Link><span className="top-note">Evidence-first literature reviews</span></header>
    <div className="container workspace"><Link className="back-link" href={`/projects/${projectId}`}>← Back to workspace</Link>
      ...
      <div className="workspace-grid">
        ...
        <section className="card section-card"><div className="section-heading"><h2>Finally included papers</h2><span className="count">{progress.includedPaperCount} included</span></div>
          ...
          {progress.papers.length === 0 ? <div className="empty">No extraction history is available yet.</div> : <div className="item-list extraction-progress-list">{progress.papers.map((item) => <Link className="item extraction-progress-item" key={item.paper.id} href={`/projects/${projectId}/extraction/${item.paper.id}`}><div className="item-row"><div><div className="item-title">{item.paper.title}</div><div className="item-meta">{item.completedRequired} / {item.requiredCount} required fields complete{!item.writeEligible ? " · read-only historical state" : ""}</div></div><div className="progress-summary"><span className={`status ${item.status === "complete" ? "supported" : "unsupported"}`}>{item.status.replace("_", " ")}</span>{item.percentage !== null && <strong>{item.percentage}%</strong>}</div></div><div className="progress-track" aria-label={`${item.percentage ?? 0}% complete`}><span style={{ width: `${item.percentage ?? 0}%` }} /></div></Link>)}</div>}
        </section>
      </div>
