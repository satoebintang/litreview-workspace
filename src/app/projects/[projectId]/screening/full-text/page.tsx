...
const states = ["all", "awaiting", "included", "excluded", "maybe", "conflict"] as const;
...
export default async function FullTextScreeningDashboardPage({ params, searchParams }: {
  ...
  const filtered = state === "all" ? queue : queue.filter(({ reviewStatus }) => state === "conflict" ? reviewStatus.crossStageConflict : state === "awaiting" ? reviewStatus.finalEligibility === "pending_full_text" : state === "included" ? reviewStatus.finalEligibility === "included" : state === "excluded" ? reviewStatus.finalEligibility === "excluded" : reviewStatus.finalEligibility === "unresolved_full_text");
  const counts = Object.fromEntries(states.slice(1).map((key) => [key, queue.filter(({ reviewStatus }) => key === "conflict" ? reviewStatus.crossStageConflict : key === "awaiting" ? reviewStatus.finalEligibility === "pending_full_text" : key === "included" ? reviewStatus.finalEligibility === "included" : key === "excluded" ? reviewStatus.finalEligibility === "excluded" : reviewStatus.finalEligibility === "unresolved_full_text").length]));
  return <main className="shell"><header className="topbar"><Link className="brand" href="/"><span className="brand-mark">T</span> Tracework</Link><span className="top-note">Evidence-first literature reviews</span></header>
    <div className="container workspace"><Link className="back-link" href={`/projects/${projectId}/screening`}>← Title/abstract screening</Link>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 18 }}><Link className="button ghost" href={`/projects/${projectId}/review-flow`}>Review flow →</Link><Link className="button ghost" href={`/projects/${projectId}/review-report`}>Review report →</Link></div>
