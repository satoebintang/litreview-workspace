...
export default async function ReviewFlowPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  return <div className="project-page">
    <div className="container workspace"><div className="workspace-header"><div><p className="eyebrow">Derived project accounting · {project.title}</p><h1>Review flow</h1><p>Live counts derived from search runs, retrieved records, Paper resolution, and screening history.</p></div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><Link className="button secondary" href={`/projects/${projectId}/review-report`}>Open review report →</Link><Link className="button secondary" href={`/projects/${projectId}/deduplication`}>Open deduplication queue →</Link></div></div>
      ...
      <section className="card section-card full"><div className="section-heading"><h2>Paper provenance classification</h2><span className="count">derived from current links</span></div><div className="screening-stats"><Metric label="Papers without historical search-acquisition links" value={summary.manualPapers} hint="includes manual, bibliographic, and PDF intake creation" /><Metric label="Historical-only acquisition Papers" value={summary.historicalAcquisitionOnlyPapers} hint="no current record link" /></div><p className="footer-note">Bibliographic records and staged PDF intakes are excluded from recorded-search identification totals. This is a review-flow summary, not a PRISMA diagram or persisted snapshot. Clickable metrics identify the relevant existing workspace population.</p></section>
    </div></div>;
}
