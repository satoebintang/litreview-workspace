...
export default async function BibliographicImportDetailPage({ params, searchParams }: { params: Promise<{ projectId: string; importId: string }>; searchParams?: Promise<{ error?: string; saved?: string; offset?: string; bulkRecordId?: string | string[] }> }) {
  ...
  return <main className="shell"><div className="container workspace">
    <Link className="back-link" href={`/projects/${projectId}/papers/imports`}>← Import history</Link>
    <div className="workspace-header"><div><p className="eyebrow">{imported.format.toUpperCase()} import</p><h1>{imported.filename}</h1><p className="hint">{records.length} records · parser {imported.parserVersion} · {imported.sourceByteSize} bytes · {imported.status}</p></div><Link className="button ghost" href={`/projects/${projectId}`}>Paper collection</Link></div>
