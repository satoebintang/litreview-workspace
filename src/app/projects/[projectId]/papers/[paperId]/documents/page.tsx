...
export default async function FullTextDocumentsPage({ params, searchParams }: { params: Promise<Params>; searchParams?: Promise<Search> }) {
  ...
  let paper;
  try {
    [, paper] = await Promise.all([reviewServices.getProject(projectId), reviewServices.getPaper(projectId, paperId)]);
  } catch (error) {
  ...
  return (
    <div className="project-page">
      <div className="container workspace"><div className="workspace-header"><div><p className="eyebrow">Full-text documents</p><h1>{paper.title}</h1><p>Immutable PDF artifacts attached to this Paper.</p></div><Link className="button ghost" href={`/projects/${projectId}`}>Workspace →</Link></div>
