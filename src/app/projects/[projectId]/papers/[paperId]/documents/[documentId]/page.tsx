...
export default async function FullTextDocumentPage({ params, searchParams }: { params: Promise<{ projectId: string; paperId: string; documentId: string }>; searchParams?: Promise<{ error?: string; saved?: string }> }) {
  ...
  return (
    <div className="project-page">
      <div className="container workspace"><div className="workspace-header"><div><p className="eyebrow">Document artifact</p><h1>{document.originalFilename}</h1><p>{paper.title}</p></div><Link className="button ghost" href={`/projects/${projectId}/papers/${paperId}/documents/${document.id}/download`}>Download PDF</Link></div>
        ...
        <div className="workspace-grid">
          <section className="card section-card"><div className="section-heading"><h2>Immutable metadata</h2><span className={`status ${document.archivedAt ? "unsupported" : "supported"}`}>{document.archivedAt ? "Archived" : "Active"}</span></div>
            ...
            {!document.archivedAt && <div style={{ marginTop: 14 }}><ConfirmAction action={archiveFullTextDocumentAction} label="Archive artifact" title="Archive this PDF artifact?" consequence="New Evidence will no longer be recorded against this artifact. Historical Evidence and the immutable bytes remain available." hiddenFields={{ projectId, paperId, documentId: document.id }} confirmLabel="Archive artifact" /></div>}
          </section>
